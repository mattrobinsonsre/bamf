...
def _patch_settings():
    """Patch settings and edge lookup to avoid DB access."""
    with (
        patch("bamf.api.routers.internal_proxy.settings") as mock_settings,
        patch(
            "bamf.api.routers.internal_proxy._lookup_edge_by_internal_token",
...
class TestVerifyInternalToken:
    ...
    async def test_token_not_configured(self, internal_client, _patch_settings):
        """If no co-located token and edge lookup fails, returns 403."""
        ...
        # With edge support, empty co-located token falls through to
        # edge lookup. If that also fails, it's 403 (not 503).
        assert resp.status_code == 403
