...
Implements the SSOConnector interface for BAMF's built-in password auth.
Instead of redirecting to an external IDP, redirects to a BAMF-hosted login
...
class LocalConnector(SSOConnector):
    ...
    build_authorization_request() returns a redirect to BAMF's own login form.
    ...
    async def build_authorization_request(
        ...
        POSTs to /api/v1/auth/local/login, which validates credentials and
