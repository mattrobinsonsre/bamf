...
|---|---|---|
| **List-envelope** `CursorPage{items,next_cursor,has_more}` | producer `services/bamf/api/models/common.py` (`CursorPage`) ↔ CLI `cmd/bamf/cmd/agents.go`, `tokens.go`, `users.go` (`Items` structs) | **Golden fixtures** `services/tests/contracts/{agents,tokens,users}_list.json` — validated by `services/tests/test_api/test_contract_fixtures.py` (producer) **and** `cmd/bamf/cmd/contract_test.go` (consumer). A key/field drift fails both. |
