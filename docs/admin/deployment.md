...
The BAMF internal CA (used for tunnel authentication) is **auto-generated by the
API server on first startup** and stored in a Kubernetes Secret (`bamf-ca`), with
a copy backed up in the database for disaster recovery. No Helm configuration is
required — there is currently no `ca.provider` chart value. See
[Certificate Management](certificates.md) for the CA lifecycle and DR procedure.
